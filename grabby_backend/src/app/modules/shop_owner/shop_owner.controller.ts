...
const createStripeConnectOnboardingLink = catchAsync(async (req: Request, res: Response) => {
  const { userId } = req.user as IReqUser;
  const result = await ShopOwnerService.createStripeConnectOnboardingLink(userId, req.body);
  sendResponse(res, {
    statusCode: 200,
    success: true,
...
const getStripeConnectStatus = catchAsync(async (req: Request, res: Response) => {
  const { userId } = req.user as IReqUser;
  const result = await ShopOwnerService.getStripeConnectStatus(userId);
  sendResponse(res, {
    statusCode: 200,
    success: true,
