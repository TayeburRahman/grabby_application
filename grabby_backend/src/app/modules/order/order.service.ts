...
import { initializeStripePayment, transferToShopOwnerAccount } from '../../../utils/stripe';
...
const processShopOwnerPayout = async (order: IOrder) => {
  // Transfer 90% payout to shop owner if paid & not yet transferred
  if (order.paymentStatus !== 'paid' || order.isTransferredToShopOwner) {
    return order;
  }
  ...
  if (!shopOwner.stripeAccountId || !shopOwner.stripeAccountConnected) {
    order.transferError = 'Shop owner has not linked a Stripe bank account yet.';
    await order.save();
    return order;
  }

  const transferAmount = Number((order.totalAmount * 0.90).toFixed(2));
  const adminFeeAmount = Number((order.totalAmount * 0.10).toFixed(2));

  const transferRes = await transferToShopOwnerAccount({
    amountInAED: transferAmount,
    destinationAccountId: shopOwner.stripeAccountId,
    orderId: order.orderId,
  });

  if (transferRes.success && transferRes.transferId) {
    order.isTransferredToShopOwner = true;
    order.stripeTransferId = transferRes.transferId;
    order.transferAmount = transferAmount;
    order.adminFeeAmount = adminFeeAmount;
    order.transferError = null;
  } else {
    order.transferError = transferRes.error || 'Failed to transfer payout to shop owner';
  }
...
const updateOrderStatus = async (orderId: string, status: string) => {
  ...
  // Create notification for Shop Owner & process payout when order is completed
  if (status === 'completed') {
    const branch = await Branch.findById(result.branchId);
    if (branch) {
      await NotificationService.createNotification({
    ...
    // Auto transfer 90% amount to shop owner's bank account (10% retained by admin)
    await processShopOwnerPayout(result);
  }
...
const retryPayout = async (orderId: string) => {
  const order = await Order.findById(orderId);
  if (!order) {
    throw new ApiError(httpStatus.NOT_FOUND, 'Order not found');
  }
  ...
  if (order.status !== 'completed') {
    throw new ApiError(httpStatus.BAD_REQUEST, 'Payout can only be transferred for completed orders');
  }

  if (order.paymentStatus !== 'paid') {
    throw new ApiError(httpStatus.BAD_REQUEST, 'Order payment status must be paid to transfer payout');
  }

  if (order.isTransferredToShopOwner) {
    throw new ApiError(httpStatus.BAD_REQUEST, 'Payout has already been transferred for this order');
  }

  const updatedOrder = await processShopOwnerPayout(order);

  if (!updatedOrder.isTransferredToShopOwner) {
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      updatedOrder.transferError || 'Failed to transfer payout to shop owner bank account'
    );
  }
